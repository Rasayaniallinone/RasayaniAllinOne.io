...
var map = {};
var dm = "https://0yjorrb4l0.execute-api.ap-south-1.amazonaws.com/";
...
function generateProduct(){
	...
	var outOfStock
	if(displayComp == "Y" ){
		outOfStock='<br><span style="'+style+'"> Quantity <input type="number" id="qtyID'+key+'" value="1" min="1" max="9" onKeyup="return checkQty(this)" size="4" style="margin-bottom: 6px;text-align: center;"></span><input type="button" onClick="return addtoCard('+key+',this)" class="btn btn-primary" value="Add to Cart">';
	}else{
		outOfStock='<br><span style="'+style+'" class="btn btn-danger">Out of Stock</span>';
	}
	var discStr = "";
	if(valuesDetails[2] == valuesDetails[1]){
		discStr='<span class="price-sale" style="'+style+'">'+valuesDetails[2]+' Rs</span>';
	}else{
		discStr='<span class="mr-2 price-dc" style="'+style+'" id="maxP'+key+'" >MRP '+maxPrice+' Rs</span><br><span class="price-sale" id="minP'+key+'" >Our Price '+minPrice+' Rs</span>';
	}
	
	var ourProducts = '<div class="col-md-6 col-lg-3 ftco-animate fadeInUp ftco-animated productNameClass" data-id="'+valuesDetails[0]+'" ><div class="product"><a href="#" class="img-prod" style="text-align: center"><img class="img-fluid" src="images/product-'+key+'.jpg" alt="UlweAllinOne" style="height: 200px;width: 200px;">'+disc +'<div class="overlay"></div></a><div class="text py-3 pb-4 px-3 text-center"><h3><a href="#">'+valuesDetails[0]+'</a></h3><div class=""><div class="pricing123"><p class="price" >'+discStr+'<span class="price-sale" >'+descDesc+'</span>'+outOfStock+'</p></div></div></div></div></div>';
...
function calcPrice(obj,key){
	$("#maxP"+key).html("MRP "+$(obj).find(':selected').attr('data-max') + " Rs");
	$("#minP"+key).html("Our Price " + $(obj).find(':selected').attr('data-min')+ " Rs");
}
